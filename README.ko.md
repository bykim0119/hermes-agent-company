...
- **Discord 오버레이** — `/code <task>`로 결정론적 생성; 실행마다 전용 스레드에
  ...
  후속 메시지는 같은 세션을 이어가고; `그만`/`stop`/`cancel`로 종료합니다.
...
- **취소:** 스레드에 `그만`을 보냅니다. `멈춰`·`중단`·`취소`·`stop`·`cancel`,
  그리고 `!stop`·`!cancel`도 같습니다. **딱 그 말만** 보냈을 때 취소로 봅니다 —
  `stop the dev server`처럼 문장 속에 섞이면 코더에게 가는 지시가 됩니다.
